...
- (void)viewInit
{
    ...
    _navTabBar = [[SCNavTabBar alloc] initWithFrame:CGRectMake(DOT_COORDINATE, 0, SCREEN_WIDTH, NAV_TAB_BAR_HEIGHT) showArrowButton:_showArrowButton];
    ...
    UIView *linve =[[UIView alloc]init];
    linve.backgroundColor =[DisplayUtil hexStringToColor:@"dedede"];
    [_navTabBar addSubview:linve];
    
    [linve mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(@1);
        make.left.equalTo(_navTabBar.mas_left);
        make.right.equalTo(_navTabBar.mas_right);
        make.bottom.equalTo(_navTabBar.mas_bottom);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    // 我的帖子页面 判断（普通 热门 精选 ）
//    if (_subViewControllers.count==3) {
//        
//        
//        [[NSNotificationCenter defaultCenter]postNotificationName:@"WSshanchu" object:page];
//        
//        
//        return;
//    }
