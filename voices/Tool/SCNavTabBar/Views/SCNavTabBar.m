...
{
//    _line = [[UIView alloc] initWithFrame:CGRectMake((SWIDTH/4-width)/2, NAV_TAB_BAR_HEIGHT - 3.0f, width , 3.0f)];
    _line = [[UIView alloc] initWithFrame:CGRectMake((SWIDTH/[_itemTitles count]-width)*0.5, NAV_TAB_BAR_HEIGHT - 3.0f, width - 4.0f, 3.0f)];
    _line.backgroundColor = [DisplayUtil hexStringToColor:@"ce3d3a"];
...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    for (NSInteger index = 0; index < [_itemTitles count]; index++)
    {
        ...
        button.frame = CGRectMake(buttonX, DOT_COORDINATE, SWIDTH/[_itemTitles count], NAV_TAB_BAR_HEIGHT);
        [button setTitle:_itemTitles[index] forState:UIControlStateNormal];
        [button setTitleColor:[DisplayUtil hexStringToColor:@"2A292E"] forState:UIControlStateNormal];
        ...
        buttonX += SWIDTH/[_itemTitles count];
        
        UIView  *linevv =[[UIView alloc]init];
        linevv.backgroundColor =[DisplayUtil hexStringToColor:@"dedede"];
        [_navgationTabBar addSubview:linevv];
     [linevv mas_makeConstraints:^(MASConstraintMaker *make) {
        
         make.width.equalTo(@1);
         make.height.equalTo(@15);
         make.left.equalTo(button.mas_right);
         make.top.equalTo(_navgationTabBar.mas_top).with.offset(15);
...
//        [[UIView alloc] initWithFrame:CGRectMake((SWIDTH/4-width - 4.0f)/2, NAV_TAB_BAR_HEIGHT - 3.0f, width - 4.0f, 3.0f)];
        
//        _line.frame = CGRectMake(button.frame.origin.x + (SWIDTH/4-[_itemsWidth[currentItemIndex] floatValue])/2, _line.frame.origin.y, [_itemsWidth[currentItemIndex] floatValue], _line.frame.size.height);
        
        
          _line.frame = CGRectMake(button.frame.origin.x + (button.frame.size.width-_line.frame.size.width)*0.5, _line.frame.origin.y, [_itemsWidth[currentItemIndex] floatValue] - 4.0f, _line.frame.size.height);
    }];
}
