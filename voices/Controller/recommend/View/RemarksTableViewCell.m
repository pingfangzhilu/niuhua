...
- (void)setCellContent:(NSString *)contentStr andIsShow:(BOOL)isShow andCellIndexPath:(NSIndexPath *)indexPath
{
    ...
    _cellIndexPath = indexPath;
     ...
     CGRect rect = [_textsLabel.text boundingRectWithSize:CGSizeMake(BOUNDS.size.width-30, 1000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]} context:nil];
    ...
    if (rect.size.height > 35) {
        ...
        if (isShow) {
            ...
            [_textsLabel mas_remakeConstraints:^(MASConstraintMaker *make){
                ...
                make.width.mas_equalTo(BOUNDS.size.width-20);
